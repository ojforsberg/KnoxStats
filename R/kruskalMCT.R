...
#'
#' @param x A numeric vector of response values (measurement variable).
#' @param g A factor or vector coercible to factor, representing the grouping variable.
#' @param conf.level Confidence level for the interval estimates. Default is 0.95.
...
kruskalMCT <- function(x, g, conf.level = 0.95, p.adjust.method = "BH") {
  ...
  if (length(unique(g)) < 2) {
    stop("'g' must have at least 2 distinct groups")
  }
  
  # Convert g to factor if it isn't already
  if (!is.factor(g)) {
    g <- as.factor(g)
  }
  
  # Display warning message
  message("\nNote: These post-hoc comparisons are only meaningful if the",
          "\n      Kruskal-Wallis test was run and yielded a significant result",
  ...
  result <- pairwise.wilcox.test(
    ...
    exact = FALSE,  # Use approximate p-values for large samples
